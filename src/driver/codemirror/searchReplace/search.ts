// code from https://github.com/zhuhs/codemirror-search-replace

import {debounce} from "ts-debounce";

export function searchReplace(CodeMirror) {
    var Search;

    CodeMirror.defineOption("searchbox", [], async function (cm) {
        cm.addKeyMap({
            "Ctrl-F": function () {
    ...
    function SearchBox(cm) {
        ...
        function init() {
            ...
            self.$onChange = debounce(() => {self.find(false, false);}, 500);
        }
        ...
        this.replace = function () {
            var readOnly = cm.getOption("readOnly"),
                isSelection = !!cm.getDoc().getSelection();
            if (!readOnly && isSelection)
                cm.getDoc().replaceSelection(this.replaceInput.value, "start");
            updateCount();
        };
